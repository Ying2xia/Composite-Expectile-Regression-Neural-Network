...
test_data_loader = DataLoader(train_data, batch_size=1)

best_loss = float('inf')
best_model = None
writer = SummaryWriter('logs')

model = CERNN(1, param_config['hidden_size'], param_config['K']).to(param_config['device'])
...
for epoch in pbar:
    ...
    for tensors, targets in train_data_loader:
        ...
        optimizer.zero_grad()
        loss = criterion(output, targets, model, param_config['l2_lambda'] / param_config['hidden_size'])
        loss.backward()
        optimizer.step()
