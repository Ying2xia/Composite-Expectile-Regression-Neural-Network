...
# change K, input_dim and distribution every time before start
param_config = {
    'K': 9,
    'epochs': 1000,
    'input_dim': 1,
    ...
    'train_num': 800,
    'batch_size': 64,
    'early_stop': 2600,  # 若200次内loss无改善则停止
    'distribution': 'chi2',
    'learning_rate': 5e-4,
    'device': 'cuda' if torch.cuda.is_available() else 'cpu'
}
train_loss = np.load(f'data/{param_config["input_dim"]}{param_config["K"]}train_{param_config["distribution"]}.npy')
test_loss = np.load(f'data/{param_config["input_dim"]}{param_config["K"]}test_{param_config["distribution"]}.npy')
...
expectile_levels = torch.tensor([k / (param_config['K'] + 1)
                                 for k in range(1, param_config['K'] + 1)]).to(param_config['device'])
...
best_model = [value for value in best_model
              if f'K={param_config["K"]}_distribution={param_config["distribution"]}' in value][0]
print(best_model)
time.sleep(0.1)

pattern_1 = re.compile(r'_hiddensize=(\d+)_')
pattern_2 = re.compile(r'_λ=(0.\d+)_')
hidden_size = pattern_1.findall(best_model)[0]
l2_lambda = pattern_2.findall(best_model)[0]
...
for _ in trange(999):
    ...
    model = CERNN(param_config['input_dim'], eval(hidden_size), param_config['K']).to(param_config['device'])
    ...
    train_data = generator[param_config['input_dim']](param_config['train_num'], expectile_levels.cpu(),
                                                      distribution=param_config['distribution'])
    test_data = generator[param_config['input_dim']](param_config['test_num'], expectile_levels.cpu(),
                                                     distribution=param_config['distribution'])
    ...
    for epoch in range(param_config['epochs']):
        for tensors, targets in train_data_loader:
            ...
            loss = criterion(output, targets, model, eval(l2_lambda) / param_config['input_dim'] / eval(hidden_size))
            loss.backward()
            optimizer.step()
...
np.save(f'data/{param_config["input_dim"]}{param_config["K"]}train_{param_config["distribution"]}.npy', train_loss)
np.save(f'data/{param_config["input_dim"]}{param_config["K"]}test_{param_config["distribution"]}.npy', test_loss)
print(f'In sample, MAE: {np.mean(train_loss[:, 0]):.6f}({np.std(train_loss[:, 0]):.6f}), '
      f'RMSE: {np.mean(train_loss[:, 1]):.6f}({np.std(train_loss[:, 1]):.6f})')
print(f'Out of sample, MAE: {np.mean(test_loss[:, 0]):.6f}({np.std(test_loss[:, 0]):.6f}), '
      f'RMSE: {np.mean(test_loss[:, 1]):.6f}({np.std(test_loss[:, 1]):.6f})')
