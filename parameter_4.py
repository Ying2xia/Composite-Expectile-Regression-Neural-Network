...
model = CERNN(4, param_config['hidden_size'], param_config['K']).to(param_config['device'])
...
for epoch in pbar:
    ...
    for tensors, targets in train_data_loader:
        ...
        optimizer.zero_grad()
        loss = criterion(output, targets, model, param_config['l2_lambda'] / param_config['hidden_size'] / 4)
        loss.backward()
        optimizer.step()
