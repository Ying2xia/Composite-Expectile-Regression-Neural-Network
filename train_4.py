...
# When K=3, epochs=1000, batch_size=64, lr=1e-3
# When K=5, epochs=1000, batch_size=64, lr=1e-3, 1-6
# When K=9, epochs=1000 ,batch_size=64, lr=2e-3, 1-7
param_config = {
    'K': 9,
    'epochs': 1000,
    'batch_size': 64,
    ...
    'learning_rate': 2e-3,
    'distribution': 't',
    'hidden_size_range': list(range(5, 8)),
    'l2_lambda_range': np.arange(0.0001, 0.0011, 0.0001),
    'device': 'cuda' if torch.cuda.is_available() else 'cpu',
    'model_save_path': 'model/Quadrivariate/K={}_distribution={}_hiddensize={}_λ={}_mae={}_rmse={}.pth'
}
...
if not os.path.exists('model'):
    os.mkdir('model')
if not os.path.exists('model/Quadrivariate'):
    os.mkdir('model/Quadrivariate')

model_files = os.listdir('model/Quadrivariate')
file = [value for value in model_files if f'K={param_config["K"]}_distribution={param_config["distribution"]}' in value]
if len(file):
    os.remove(os.path.join('model/Quadrivariate', file[0]))

expectile_levels = torch.tensor([k / (param_config['K'] + 1) for k in range(1, param_config['K'] + 1)],
                                device=param_config['device'])
train_data = QuadrivariateData(param_config['train_num'], expectile_levels.cpu(), param_config['distribution'])
...
for hidden_size, l2_lambda in product(param_config['hidden_size_range'], param_config['l2_lambda_range']):
    ...
    model = CERNN(4, hidden_size, param_config['K']).to(param_config['device'])
    ...
    for epoch in pbar:
        ...
        for tensors, targets in train_data_loader:
            ...
            optimizer.zero_grad()
            loss = criterion(output, targets, model, l2_lambda / hidden_size / 4)
            loss.backward()
            optimizer.step()
